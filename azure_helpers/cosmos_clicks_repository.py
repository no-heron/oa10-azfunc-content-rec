...
def get_all_clicks() -> pd.DataFrame:
    ...
    try:
        ...
        if not results:
            logging.info("No click records found.")
            return df
        return pd.concat([df, pd.DataFrame(results)])

    except exceptions.CosmosHttpResponseError as e:
        logging.error("Cosmos DB query error in get_all_clicks: %s", e)
        raise
    except Exception as e:
        logging.exception("Unexpected error retrieving clicks: %s", e)
        raise
...
def get_clicked_articles_by_user(user_id: int) -> List[int]:
    ...
    if not isinstance(user_id, int):
        logging.warning("Invalid user_id provided to get_clicked_articles_by_user: %s", user_id)
        return []
    ...
    try:
        ...
        logging.debug("User %s clicked %d articles.", user_id, len(articles))
        return list[int](articles)
    except exceptions.CosmosHttpResponseError as e:
        logging.error("Cosmos DB query error in get_clicked_articles_by_user: %s", e)
        raise
    except Exception as e:
        logging.exception("Unexpected error retrieving user clicks: %s", e)
        raise
...
def get_last_clicked_by_user(user_id: int) -> Optional[int]:
    ...
    """
    if not isinstance(user_id, int):
        logging.warning("Invalid user_id provided to get_last_clicked_by_user: %s", user_id)
        return None
    ...
    try:
        ...
        logging.debug("User %s last clicked article: %s", user_id, last_click)
        return last_click
    except exceptions.CosmosHttpResponseError as e:
        logging.error("Cosmos DB query error in get_last_clicked_by_user: %s", e)
        raise
    except Exception as e:
        logging.exception("Unexpected error retrieving last click: %s", e)
        raise